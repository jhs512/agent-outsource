...
Run the included Node.js program by its absolute path. Node.js 24 or newer is required. Shared state is `~/.agent-outsource/agent-outsource.sqlite` and logs are `~/.agent-outsource/agent-outsource.sqlite.logs/`, across repositories for the same OS user. Only this dedicated directory needs Codex workspace-write access. Old home-level databases migrate after all old services/connections close; conflicting databases require reconciliation. Never interrupt active workers merely to migrate. `--db` is for isolated verification only. For sandbox setup and existing-task limitations, see [the setup guide](https://github.com/jhs512/agent-outsource/blob/main/docs/setup.md).
