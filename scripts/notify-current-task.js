...
const idleConfirmation = process.argv[3] === 'idle-confirmation';
...
record({ phase: 'scheduled', delay, idleConfirmation });
...
try {
  ...
  const result = await client.callTool({ name: 'send_message_to_thread', arguments: {
    threadId,
    prompt: idleConfirmation
      ? '외부 알림 두 번째 실험 결과: 음성 세션이 종료된 뒤 별도 Node.js 프로세스가 대기했다가 현재 작업에 이 메시지를 보냈습니다. 첫 번째 실험과 달리 음성 종료 시 전달되는 메시지와 겹치지 않도록 분리한 유휴 상태 재실행 확인입니다. 실제 작업자 완료 알림은 아닙니다.'
      : '외부 알림 실험: ai-oc의 별도 Node.js 프로세스가 대기 후 이 메시지를 현재 작업에 전달했습니다. 작업자 실행 완료 알림을 흉내 낸 단발 테스트입니다. 실제 작업자 작업 완료를 의미하지 않습니다.',
